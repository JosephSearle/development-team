...
| **AsyncSubAgent** | A DeepAgents ≥0.6.1 construct that spawns a named sub-agent as an in-process ASGI graph, enabling parallel sub-task execution within a parent agent's context window. Used by the Architecture Agent (`library_researcher`) and Incident Response Agent (`log_analyzer`, `metrics_analyzer`). |
...
| **FilesystemBackend** | A DeepAgents ≥0.6.1 class that backs agent file I/O with real on-disk reads and writes (via `read_file`, `write_file`, `grep`, `glob` built-in tools). File-writing agents (Code, Test, Architecture, Documentation, Infrastructure) use `FilesystemBackend` pointed at their workspace directory. The orchestrating node reads result files back from disk rather than extracting them from AI message strings. |
...
| **DeepAgents** | The LangChain agent harness (`pip install deepagents>=0.6.1`) used for all twelve specialist agents. Version 0.6.x provides: `create_deep_agent` factory, `FilesystemBackend` (real file I/O), `FilesystemPermission` (path-scoped allow/deny), `SummarizationMiddleware` (automatic context management), `skills=` parameter (SKILL.md progressive-disclosure expertise), and `AsyncSubAgent` (parallel sub-task delegation). Every specialist agent in this system is a `create_deep_agent` instance. |
...
| **Milvus** | A purpose-built vector database deployed as a StatefulSet in the `dev-team-platform` namespace. Not used in the agent runtime path: agents perform codebase search via DeepAgents' built-in `grep`/`glob` tools on live files. Milvus is retained in infrastructure for potential future use (e.g., semantic code similarity at scale). Activating it as an agent data source requires a new ADR. |
...
| **Orchestration Layer** | The capability group comprising the Orchestrator Agent. The single entry point for all tasks; decomposes requirements, manages the HITL gate, and routes work to all other agents. |
...
| **Skills** | Per-agent domain expertise bundled as `SKILL.md` files within each agent's Python package at `agents/<name>/src/<module>/skills/<skill-name>/SKILL.md`. DeepAgents ≥0.6.1 loads them via the `skills=` parameter at agent instantiation. SKILL.md frontmatter declares the skill name, a ≤1024-char description the LLM uses to decide relevance, and the allowed-tools list. Skills provide progressive-disclosure domain expertise without padding the system prompt unconditionally. |
| **SCA (Software Composition Analysis)** |
| **SummarizationMiddleware** | A DeepAgents ≥0.6.1 middleware that automatically summarises the agent's conversation history when it approaches the model's context limit. Two variants: `deepagents.middleware.SummarizationMiddleware` (requires a `FilesystemBackend` for history offloading — used by file-writing agents) and `langchain.agents.middleware.SummarizationMiddleware` (no backend — used by reasoning-only agents). Applied to every specialist agent. | Automated analysis of third-party dependencies for known vulnerabilities (CVEs). Performed by the Security Agent and Dependency Agent via SonarQube MCP. |
