...
<!-- Version numbers reflect the implemented system as of May 2026. Update this table when versions are pinned in pyproject.toml / uv.lock / Helm values files. -->
...
|---|---|---|---|
| **Agent Harness** | LangChain DeepAgents | 0.6.2 (`>=0.6.1`) | Batteries-included agent harness built on LangGraph. Version 0.6.x introduces: `FilesystemBackend` (real file I/O without string extraction), `FilesystemPermission` (path-scoped allow/deny rules), `SummarizationMiddleware` (auto context management with backend offloading), `skills=` parameter (SKILL.md progressive-disclosure domain expertise), and `AsyncSubAgent` (parallel sub-task delegation). MIT licensed. See [ADR-0002](adr/0002-adopt-deepagents-langgraph-for-agent-orchestration.md) |
...
| **Vector Store** | Milvus | latest stable | Purpose-built vector database deployed in the `dev-team-platform` namespace as a platform resource. Not used in the agent runtime path: live codebase search is performed by DeepAgents' built-in `grep`/`glob` tools via `FilesystemBackend`. Reserved for future semantic-search use cases if needed. |
...
| **Database Migrations** | Alembic | latest | Manages relational schema migrations if relational stores are introduced. Not currently used in the agent runtime path. If Milvus is brought into the agent runtime in future, a collection migration strategy must be defined. |
